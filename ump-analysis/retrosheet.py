...
teamfile_url = 'https://raw.githubusercontent.com/chadwickbureau/retrosheet/master/seasons/{}/TEAM{}'
...
def teamfile(season, export_dir = './'):
    """
    Pull and Download Retrosheet teamfile for a given season
    """
    GH_TOKEN=os.getenv('GH_TOKEN', '')
    # validate input
    g = Github(GH_TOKEN)
    repo = g.get_repo('chadwickbureau/retrosheet')
    season_folder = [f.path[f.path.rfind('/')+1:] for f in repo.get_contents(f'seasons/{season}')]
    filename = f'TEAM{season}'

    if filename not in season_folder:
        raise ValueError(f'teamfile not available for {season}')
    s = get_text_file(teamfile_url.format(season, season))
    with open(os.path.join(export_dir, filename), 'w') as f:
        f.write(s)